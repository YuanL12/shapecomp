...
#include "geomp/mesh/Vertex.h"
#include "geomp/mesh/Edge.h"
#include "geomp/mesh/Face.h"
#include "geomp/mesh/Corner.h"

namespace geomp {

// compute the barycentric coordinates of a point P
// note the return value can be negative if P is outside the triangle
inline std::tuple<double, double, double> barycentric_coordinates(
    const geomp::Vector& A, 
    const geomp::Vector& B, 
    const geomp::Vector& C, 
    const geomp::Vector& P)
{
    // Compute reference normal for ABC
    geomp::Vector N = cross(B - A, C - A);
    
    // Compute sub-triangle normals
    geomp::Vector N_A = cross(B - P, C - P);
    geomp::Vector N_B = cross(C - P, A - P);
    geomp::Vector N_C = cross(A - P, B - P);

    // Compute barycentric coordinates
    double denom = dot(N, N);  // ||N||^2

    double lambdaA = dot(N_A, N) / denom;
    double lambdaB = dot(N_B, N) / denom;
    double lambdaC = dot(N_C, N) / denom;

    // Each weight is computed with a separate division by denom, so the sum can drift from 1
    // (often ~1e-5 on large UVs or skinny triangles). For coplanar A,B,C,P the true weights sum
    // to 1; we allow them to happen.
    
    return std::make_tuple(lambdaA, lambdaB, lambdaC);
}
